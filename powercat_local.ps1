function powercat
{
  ...
  ############### HELP ###############
  $Help = "
local powercat - Netcat, The Powershell Version

Note: this script has been modified to run on localhost for the client.
##  $Socket = New-Object System.Net.Sockets.TcpListener("127.0.0.1", $p)  ##

Originating github repository: https://github.com/besimorhino/powercat
...
"
  ...
  function Setup_TCP
  {
    ...
    {
      $FuncVars["l"] = $True
      Write-Verbose ("Listening on [127.0.0.1] (port " + $p + ")")
      $Socket = New-Object System.Net.Sockets.TcpListener("127.0.0.1", $p)
      $Socket.Start()
